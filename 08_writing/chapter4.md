...
- Canonical run-ID authority: `07_implementation/ACTIVE_BASELINE.md` (all chapter run references should resolve through this baseline registry)
...
| --- | --- | --- | --- | --- | --- |
| `EP-REPRO-001` | `TC-003` | `pass` | `deterministic_match=true`, `status=pass`, `replay_count=3` | `07_implementation/implementation_notes/bl010_reproducibility/outputs/bl010_reproducibility_report.json`, `07_implementation/implementation_notes/bl010_reproducibility/outputs/bl010_reproducibility_run_matrix.csv` | Stable recommendation artifacts replay identically; raw JSON hash variance remains metadata-driven (`run_id`, timestamps). |
| `EP-EXPL-001` | `TC-004` | `pass` | `playlist_track_count=10`, `top_contributor_mix={Lead genre:4, Tag overlap:3, Genre overlap:3}`, `explanation_count=10` | `07_implementation/implementation_notes/bl008_explanation/outputs/bl008_explanation_summary.json`, `07_implementation/implementation_notes/bl008_explanation/outputs/bl008_explanation_payloads.json` | All playlist tracks carry mechanism-linked explanation payloads with explicit top-contributor attribution. |
| `EP-CTRL-001` | `TC-005` | `pass` | `scenario=no_influence_tracks`, `top10_overlap_ratio=1.0`, `mean_abs_rank_delta=0.0`, `status=pass` | `07_implementation/implementation_notes/bl011_controllability/outputs/bl011_controllability_report.json` (scenario `no_influence_tracks`) | Influence-track toggle is repeat-consistent and auditable; this run produced no rank/playlist displacement under current data profile. |
| `EP-CTRL-002` | `TC-006` | `pass` | `scenario=valence_weight_up`, `top10_overlap_ratio=0.5`, `playlist_overlap_ratio=0.5`, `mean_abs_rank_delta=2876.321` | `07_implementation/implementation_notes/bl011_controllability/outputs/bl011_controllability_report.json` (scenario `valence_weight_up`) | Feature-weight actuation yields substantial, directionally consistent re-ranking and playlist turnover. |
| `EP-CTRL-003` | `TC-007` | `pass` | `stricter_delta=-10476`, `looser_delta=+8990`, `all_variant_shifts_observable=true` | `07_implementation/implementation_notes/bl011_controllability/outputs/bl011_controllability_report.json` (scenarios `stricter_thresholds`, `looser_thresholds`) | Threshold scaling changes candidate-pool width as expected while preserving deterministic repeat consistency. |
| `EP-RULE-001` | `TC-008` | `pass` | `length_target=10`, `length_actual=10`, `undersized=false` | `07_implementation/implementation_notes/bl007_playlist/outputs/bl007_assembly_report.json`, `07_implementation/implementation_notes/bl007_playlist/outputs/bl007_playlist.json` | Playlist-length rule is satisfied on the active run; no undersized warning triggered. |
| `EP-RULE-002` | `TC-008` | `pass` | `max_per_genre=4`, `max_consecutive=2`, `R2_genre_cap_hits=2859`, `R3_consecutive_hits=3907` | `07_implementation/implementation_notes/bl007_playlist/outputs/bl007_assembly_report.json`, `07_implementation/implementation_notes/bl007_playlist/outputs/bl007_assembly_trace.csv` | Rule-pressure diagnostics confirm constraints are actively enforced rather than bypassed. |
| `EP-OBS-001` | `TC-009` | `pass` | `schema=bl009-observability-v1`, `canonical_config_artifact_pair_available=true` | `07_implementation/implementation_notes/bl009_observability/outputs/bl009_run_observability_log.json`, `07_implementation/implementation_notes/bl009_observability/outputs/bl009_run_index.csv` | Run-level observability surface includes stage linkage, hashes, config provenance, and execution-scope summary. |
| `EP-ALIGN-001` | `TC-010` | `pass` | `matched_isrc=1098`, `matched_fallback=806`, `unmatched_rate=0.8405`, `match_rate=0.1595` | `07_implementation/implementation_notes/bl003_alignment/outputs/bl003_ds001_spotify_summary.json`, `07_implementation/implementation_notes/bl003_alignment/outputs/bl003_ds001_spotify_trace.csv` | Alignment miss rate remains high but explicitly measured and reported as a bounded design limitation. |
| `EP-QUAL-001` | `TC-011` | `pass` | `sanity_pass=22/22`, `freshness_pass=19/19` | `07_implementation/implementation_notes/bl014_quality/outputs/bl014_sanity_report.json`, `07_implementation/implementation_notes/bl014_quality/outputs/bl_active_freshness_suite_report.json` | Automated quality gates confirm active baseline consistency across sanity and freshness checks. |
...
| Sanity and freshness quality gates | `BL014-SANITY-20260327-201731-408637` (`bl014_sanity_report.json`) | `BL-FRESHNESS-SUITE-20260327-201942` (`bl_active_freshness_suite_report.json`) | `pass` | BL-014 confirms run consistency with 22/22 sanity checks and 19/19 freshness checks on the active baseline. |

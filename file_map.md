...
| `07_implementation/README.md` | active | `07_implementation/` | Canonical runtime-package orientation file for the standalone implementation surface. | Updated to reflect the package posture and current modularized `src/` layout. |
| `07_implementation/main.py` | active | `07_implementation/` | Standalone package entrypoint that launches the deterministic stage pipeline. | Retained as the stable operator-facing entry surface while internal orchestration helpers are split into focused modules under `src/orchestration/`. |
| `07_implementation/src/orchestration/main.py` | active | `07_implementation/src/orchestration/` | Thin BL-013 orchestration entrypoint. | Reduced to wrapper/orchestration flow control over `cli.py`, `seed_freshness.py`, `stage_runner.py`, and `summary_builder.py` to remove monolithic control logic from one file. |
| `07_implementation/src/orchestration/cli.py` | active | `07_implementation/src/orchestration/` | Parses BL-013 arguments and validates requested stage order. | Added during Phase 5 modularization so orchestration CLI handling is isolated from run execution and summary logic. |
| `07_implementation/src/orchestration/seed_freshness.py` | active | `07_implementation/src/orchestration/` | Verifies BL-003 seed freshness against the active effective run-config. | Added during Phase 5 modularization to isolate seed-contract validation from the BL-013 entrypoint. |
| `07_implementation/src/orchestration/stage_runner.py` | active | `07_implementation/src/orchestration/` | Executes BL stage subprocesses and builds missing-script failure payloads. | Added during Phase 5 modularization to isolate subprocess execution concerns from CLI and summary assembly. |
| `07_implementation/src/orchestration/summary_builder.py` | active | `07_implementation/src/orchestration/` | Builds BL-013 run summaries and stable artifact diagnostics. | Added during Phase 5 modularization so hashing/summary logic remains reusable and separate from stage execution. |
| `07_implementation/src/controllability/pipeline_runner.py` | active | `07_implementation/src/controllability/` | Thin BL-011 scenario orchestrator. | Reduced to stage composition over the dedicated profile, retrieval, scoring, and playlist executors while preserving the stable public entry surface. |
| `07_implementation/src/controllability/scenarios.py` | active | `07_implementation/src/controllability/` | BL-011 scenario construction entrypoint. | Reduced to scenario assembly over extracted `pathing.py` and `runtime_controls.py` helpers so path resolution and runtime policy are no longer embedded in one file. |
| `07_implementation/src/controllability/pathing.py` | active | `07_implementation/src/controllability/` | Resolves required BL-011 input and output paths. | Added during Phase 6 modularization to isolate filesystem/path contract checks from scenario construction logic. |
| `07_implementation/src/controllability/runtime_controls.py` | active | `07_implementation/src/controllability/` | Resolves BL-011 run-config driven runtime controls and scenario policy. | Added during Phase 6 modularization so control resolution is separate from scenario assembly. |
| `07_implementation/src/controllability/stage_profile.py` | active | `07_implementation/src/controllability/` | Executes the BL-004 profile stage for BL-011 scenarios. | Added during Phase 6 modularization to isolate profile aggregation logic and stable hashes from the pipeline orchestrator. |
| `07_implementation/src/controllability/stage_retrieval.py` | active | `07_implementation/src/controllability/` | Executes the BL-005 retrieval/filtering stage for BL-011 scenarios. | Added during Phase 6 modularization to isolate candidate keep/reject decisions and diagnostics. |
| `07_implementation/src/controllability/stage_scoring.py` | active | `07_implementation/src/controllability/` | Executes the BL-006 scoring stage for BL-011 scenarios. | Added during Phase 6 modularization to isolate component scoring and summary logic. |
| `07_implementation/src/controllability/stage_playlist.py` | active | `07_implementation/src/controllability/` | Executes the BL-007 playlist assembly stage for BL-011 scenarios. | Added during Phase 6 modularization to isolate playlist rule evaluation and trace/report output generation. |
| `07_implementation/src/alignment/matching.py` | active | `07_implementation/src/alignment/` | Thin compatibility wrapper for BL-003 matching helpers. | Reduced to a stable import surface over the extracted text matching, indexing, and match-pipeline helpers. |
| `07_implementation/src/alignment/reporting.py` | active | `07_implementation/src/alignment/` | Thin compatibility wrapper for BL-003 reporting helpers. | Reduced to a stable import surface over extracted writers, validation, and summary-builder helpers. |
| `07_implementation/src/alignment/text_matching.py` | active | `07_implementation/src/alignment/` | Normalizes text and resolves exact/fuzzy candidate selection helpers for BL-003. | Added during Phase 6 modularization to isolate string matching and fuzzy-control logic. |
| `07_implementation/src/alignment/index_builder.py` | active | `07_implementation/src/alignment/` | Builds DS-001 Spotify/title/artist lookup indices for BL-003. | Added during Phase 6 modularization to isolate candidate indexing from per-event matching flow. |
| `07_implementation/src/alignment/match_pipeline.py` | active | `07_implementation/src/alignment/` | Runs the per-event BL-003 alignment loop and produces match traces. | Added during Phase 6 modularization to isolate the main matching loop from low-level text and reporting helpers. |
| `07_implementation/src/alignment/writers.py` | active | `07_implementation/src/alignment/` | Writes BL-003 CSV/JSONL artifacts and source-scope manifests. | Added during Phase 6 modularization to isolate output serialization from summary assembly and matching logic. |
| `07_implementation/src/alignment/validation.py` | active | `07_implementation/src/alignment/` | Enforces BL-003 match-rate validation. | Added during Phase 6 modularization to keep validation rules small and reusable. |
| `07_implementation/src/alignment/summary_builder.py` | active | `07_implementation/src/alignment/` | Builds and writes the BL-003 summary JSON. | Added during Phase 6 modularization to isolate summary assembly from reporting I/O details. |

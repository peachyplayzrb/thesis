...
| `EP-CTRL-001` | Influence tracks | Add/remove defined influence-track set | `top_k_overlap_delta`, `rank_shift_summary` | baseline/variant ranking snapshots | Directional shift is observable and explainable from profile/score traces |
...
| `EP-ALIGN-001` | Alignment behavior visibility | ISRC-first with fallback enabled | `matched_isrc`, `matched_fallback`, `unmatched_rate` | alignment summary + unmatched diagnostics | Match-path counts are reported and unmatched cases include explicit reason categories |

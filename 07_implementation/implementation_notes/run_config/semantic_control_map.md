...
| 2 | Profile Construction | `profile_controls` | BL-004 |
| 3 | Retrieval Filtering | `retrieval_controls` | BL-005 |
| 4 | Scoring | `scoring_controls` | BL-006 |
| 5 | Playlist Assembly | `assembly_controls` | BL-007 |
| 6 | Transparency | `transparency_controls` | BL-008 |
| 7 | Observability | `observability_controls` | BL-009 |
...
**Consuming stages:**
- BL-003 (`build_bl003_seed_refresh.py`): applies source-scope filters and produces the enriched seed manifest; emits `bl003_source_scope_manifest.json`
...
**Resolver:** `resolve_input_scope_controls(run_config_path)` → `run_config_utils.py:resolve_bl004_controls`

---

## Group 2 — Profile Construction
...
| `resolve_bl004_controls` | 1, 2 | BL-004 |
| `resolve_bl005_controls` | 3 | BL-005 |
| `resolve_bl006_controls` | 4 | BL-006 |
| `resolve_bl007_controls` | 5 | BL-007 |
| `resolve_bl008_controls` | 6 | BL-008 |
| `resolve_bl009_controls` | 7 | BL-009 |
...
|---|---|---|
| `run_config/outputs/run_intent_<timestamp>.json` | `run-intent-v1` | The requested configuration as provided (or implicit default if no run-config file was supplied) |
| `run_config/outputs/run_effective_config_<timestamp>.json` | `run-effective-config-v1` | The fully resolved, normalized configuration after all coercions and defaults are applied |
...
The following controllability scenarios are documented in `07_implementation/implementation_notes/controllability/outputs/scenarios/` and each modifies one or more groups:

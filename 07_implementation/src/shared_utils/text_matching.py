...
def _fuzzy_diagnostics_payload(
    *,
    failure_reason: str,
    album_score: float | None,
    artist_match_count: int,
    candidate_count_after_artist_filter: int,
) -> dict[str, Any]:
    return {
        "album_score": album_score,
        "artist_match_count": artist_match_count,
        "candidate_count_after_artist_filter": candidate_count_after_artist_filter,
        "failure_reason": failure_reason,
    }


def _artist_matches_above_threshold(
    *,
    artist_key: str,
...
def _dedupe_candidates_by_best_artist_score(
    *,
...
def _score_candidate_match(
    *,
    ...
    if title_score < fuzzy_controls["title_threshold"]:
        return None, None, 0.0, "title_threshold"
    ...
    if combined_score < fuzzy_controls["combined_threshold"]:
        return None, None, album_score, "combined_threshold"
    ...
    if event_duration is not None and candidate_duration is not None:
        ...
        if duration_delta > fuzzy_controls["max_duration_delta_ms"]:
            return None, None, album_score, "duration_rejected"
...
def fuzzy_find_candidate(
    *,
    ...
    artist_matches = _artist_matches_above_threshold(
        artist_key=artist_key,
        artist_keys=artist_keys,
        fuzzy_controls=fuzzy_controls,
    )
    ...
    deduped_candidates = _dedupe_candidates_by_best_artist_score(
        by_artist=by_artist,
    ...
    for ds001_id in sorted(deduped_candidates.keys()):
        ...
        choice, sort_key, album_score, failure_reason = _score_candidate_match(
            candidate=candidate,
            ds001_id=ds001_id,
        ...
        if failure_reason == "title_threshold":
            saw_title_threshold_failure = True
            continue
        if failure_reason == "combined_threshold":
            saw_combined_threshold_failure = True
            continue
        if failure_reason == "duration_rejected":
            saw_duration_rejection = True
            continue
        if choice is None or sort_key is None:
            continue

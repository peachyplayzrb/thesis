...
- BL-019 planning addendum (2026-03-21): implement the DS-002 candidate-corpus workflow (`MSD subset + Last.fm tags + MusicBrainz mapping`) with deterministic joins, manifest output, and quality-gate checks before downstream reruns.
...
- Corpus volatility risk: the canonical candidate dataset may change while implementation is in progress.
	- Control: lock BL-019 to DS-002 source contracts (`MSD subset`, `Last.fm tags`, `MusicBrainz mapping`) and record manifest hashes for each refresh run.

"""BL-013 orchestration entry point that runs BL-003 through BL-009 in order."""
...
def main() -> None:
    ...
    # Resolve run_config_path first because orchestration controls depend on it.
    ...
    # Pull orchestration controls from run-config, then fall back to defaults.
    ...
    # Precedence is CLI flags first, then run-config controls, then hard defaults.
    ...
    else:  # Treat unknown policy values the same as auto_if_stale.
    ...
    # The freshness guard only runs when policy allows it and no refresh is requested.

"""BL-009 observability entry point.

This stage records run-chain diagnostics and artifact linkage so BL-004 through
BL-008 outputs can be audited as one coherent execution story.
"""
...
def build_signal_mode_calibration_summary(
    ...
    """Build a compact signal-mode calibration snapshot from BL-005 and BL-006 summaries."""
...
def first_items(values: list[dict[str, str]], limit: int) -> list[dict[str, str]]:
    """Return the first `limit` rows for small sampled diagnostics blocks."""

    return values[:limit]


def parse_exclusion_samples(rows: list[dict[str, str]], field: str, limit: int) -> dict[str, list[dict[str, str]]]:
    """Group sampled exclusion rows by reason field, capped per reason."""
...
def build_source_resilience_diagnostics(bl003_summary: dict[str, object]) -> dict[str, object]:
    """Derive source-resilience status and per-source decisions from BL-003 summary data."""
...
def ensure_required_sections(run_log: dict) -> None:
    """Fail fast if the final BL-009 run log is missing required top-level sections."""
...
def resolve_canonical_config_artifacts(
    ...
    """Resolve canonical config artefact paths and hash availability from runtime controls."""
...
def build_artifact_maps(
    ...
    """Build relative-path hash and size maps for non-script input/output artefacts."""
...
def main() -> None:
    ...
    # Require all upstream artefacts before any observability synthesis starts.
    ...
    # This compact block makes it easier to compare runs that used different signal modes.

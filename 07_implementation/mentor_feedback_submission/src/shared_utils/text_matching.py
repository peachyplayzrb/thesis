"""Text normalization and fuzzy-matching helpers used during BL-003 alignment."""
...
def _compute_ratio(left: str, right: str) -> float:
    try:
        # Prefer rapidfuzz when available, but keep a stdlib fallback so the bundle still runs.
...
def normalize_text(value: str) -> str:
    """Normalize a string into ASCII lower-case tokens for fuzzy comparison."""
...
def choose_best_duration_match(
    ...
    """Pick the candidate with the closest duration, or fall back to the first row."""
    if not candidates:

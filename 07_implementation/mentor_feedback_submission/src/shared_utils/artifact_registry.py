"""
This file keeps the canonical list of stage scripts and artifact paths.

I use it anywhere the pipeline needs to refer to outputs by name so those paths
do not get redefined in multiple places.
"""
...
def stage_specs() -> List[Dict[str, str]]:
    """Return the stage metadata in the fixed pipeline order."""
    return [dict(spec) for spec in _STAGE_SPECS]


def artifact_summary_targets(src_root: Path) -> Dict[str, Path]:
    """Build the absolute paths used by status views and artifact summaries."""
    return {
...
def bl013_stage_script_map() -> Dict[str, str]:
    """Map the BL-013 stage labels to the scripts they should run."""
...
def bl014_bl013_latest_summary_path(repo_root: Path) -> Path:
    """Return the latest BL-013 summary file used by BL-014."""
...
def bl014_pipeline_script_paths(repo_root: Path) -> Dict[str, Path]:
    """Return the stage scripts BL-014 calls during refresh checks."""
    return {
...
def bl014_refinement_diagnostic_paths(repo_root: Path) -> Dict[str, Path]:
    """Return the BL-006 and BL-007 diagnostics BL-014 inspects."""
    return {
...
def bl003_required_paths(repo_root: Path) -> Dict[str, Path]:
    """Return the BL-003 outputs that downstream stages depend on."""
    return {
...
def bl010_required_paths(repo_root: Path) -> Dict[str, Path]:
    """Return the scripts and artifacts BL-010 expects to find."""
    return {
...
def bl009_required_paths(repo_root: Path, bl009_script_path: Path | None = None) -> Dict[str, Path]:
    """Return the scripts and artifacts BL-009 expects to be present."""

...
def decide_candidate(
    *,
    ...
    if len(playlist) >= target_size:
        # R4 enforces the final playlist size contract.
        rule_hits["R4_length_cap"] += 1
        return "excluded", "length_cap_reached"

    if not ignore_genre_cap:
        # R2 prevents one dominant genre from taking over the entire list.
        if sum(
            1
    ...
    if not ignore_consecutive:
        # R3 breaks up long same-genre streaks so the final list stays varied.

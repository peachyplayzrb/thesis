...
**Blocking**: Resolved (2026-04-19). All 8 code review findings (CR-1 through CR-8) now complete. Core pipeline is green with improved type-safety (CR-1), reproducibility auditability (CR-2), code maintainability (CR-3, CR-5, CR-6, CR-7, CR-8), and error visibility (CR-4).
...
### UNDO-S: Code review findings — correctness, reproducibility, and legibility gaps

**Status**: Resolved (2026-04-20)

**Trigger**: Manual code review of `07_implementation/src/` surface identified five issues ranging from high-severity correctness/reproducibility risks to medium/low legibility and performance gaps.

**Description**: Track all review findings as explicit action items. Items are ordered by severity.

**Checklist**

**High severity**
- [x] `CR-1`: `_numeric_scores` in `retrieval/candidate_evaluator.py` returns a 9-element positional tuple. Unpacking at the call site is order-sensitive with no type guard — a positional swap (e.g., `numeric_support_score_weighted` vs `numeric_support_score_weighted_absolute`) silently produces wrong filtering decisions. Replace with a `NamedTuple` or `@dataclass` return type. **FIXED 2026-04-19**: Implemented `NumericScores` NamedTuple with all 9 fields properly typed. Call sites continue to work via tuple unpacking compatibility (C-567).
- [x] `CR-2`: `BL_REFERENCE_NOW_UTC` environment variable in `alignment/weighting.py` (`_resolve_reference_now_utc`) is an implicit control channel that bypasses the formal run-config system. If set in a CI/shell environment it silently alters temporal decay without appearing in any run-summary artifact — directly undermining determinism and reproducibility claims. Log any env-var override in BL-013 stage diagnostics (or reject it in favor of run-config authority). **FIXED 2026-04-19**: Added explicit `warnings.warn()` when env-var is used, with clear auditability message directing to run-artifact tracking (C-567).

**Medium severity**
- [x] `CR-3`: `weighted_lead_genre_similarity` is defined independently (and identically) in both `retrieval/candidate_evaluator.py` and `scoring/scoring_engine.py`. Neither imports the other. If the two implementations ever diverge, retrieval silently uses stale scoring logic. Consolidate into one authoritative location and import from there. **FIXED 2026-04-19**: Consolidated into `scoring/scoring_engine.py` as authoritative location; removed duplicate from `retrieval/candidate_evaluator.py` and added import. Call site remains functional via import substitution (C-568).
- [x] `CR-4`: `_apply_preference_weight_policy` in `alignment/aggregation.py` falls through to `sum` behavior for any unknown mode string (e.g., a YAML typo or case mismatch). An invalid mode goes undetected and produces wrong preference weights. Add an explicit `else: raise ValueError` branch so bad run-config inputs fail fast. **FIXED 2026-04-19**: Added explicit error branch that raises `ValueError` with clear message for invalid modes; all valid modes (sum, max, mean, capped) continue to work. Error handling verified via code execution test (C-568).
- [x] `CR-NEW16`: `_semantic_scores` in `retrieval/candidate_evaluator.py` returned a 7-element positional tuple (semantic overlap and score fields), keeping a residual order-fragility surface after earlier NamedTuple hardening. **FIXED 2026-04-20**: Converted to `_SemanticScores` NamedTuple with explicit field names; focused retrieval tests and pyright checks pass (C-574).

**Low severity / legibility**
- [x] `CR-5`: `decide_candidate` in `playlist/rules.py` scans the entire playlist with a `sum(... for track in playlist)` on every candidate evaluation. For large candidate pools this is O(candidates × playlist_size). Maintain a per-genre `Counter` alongside the playlist to reduce the per-check cost to O(1). **FIXED 2026-04-19**: Added optional `genre_counts: Counter[str] | None` parameter to `decide_candidate()`; when provided, uses O(1) Counter lookup; falls back to legacy O(n) scan when None for backward compatibility. Call sites can optionally maintain a Counter as playlist is built (C-569).
- [x] `CR-6`: `_compute_ratio` in `shared_utils/text_matching.py` calls `import_module("rapidfuzz.fuzz")` on every invocation. Python's module cache means the disk load only happens once, but the `import_module` lookup plus `getattr` overhead runs per fuzzy comparison. Hoist the try/except import to module level. **FIXED 2026-04-19**: Module-level initialization of `_FUZZ_MODULE` and `_FUZZ_RATIO_FN` globals at import time; `_compute_ratio()` uses cached result, eliminating per-call overhead (C-569).
- [x] `CR-7`: The `fuzz` compatibility shim class in `shared_utils/text_matching.py` is named in lowercase, violating PEP 8 class naming convention. Rename to `_FuzzCompat` or similar to avoid confusion with module-level variables. **FIXED 2026-04-19**: Renamed class to `_FuzzCompat`; exported as `fuzz = _FuzzCompat` for backward compatibility. All call sites continue to work (C-569).
- [x] `CR-8`: Score-band phrase thresholds (`0.75`, `0.5`) in `transparency/explanation_driver.py` (`build_why_selected`) are inlined magic numbers that tie directly to score-band interpretation. If score normalization changes, the phrases silently become wrong. Promote to named module-level constants. **FIXED 2026-04-19**: Promoted to named constants `_STRONG_MATCH_THRESHOLD = 0.75` and `_MODERATE_MATCH_THRESHOLD = 0.5`; updated all uses in `build_why_selected()` to reference constants instead of literals (C-569).

**Blocking**: Resolved. Core pipeline contracts are green; UNDO-S and Phase A follow-up tuple-fragility closure are complete.

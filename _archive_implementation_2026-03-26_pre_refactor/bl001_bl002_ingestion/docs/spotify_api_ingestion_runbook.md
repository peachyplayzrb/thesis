...
## Run Command
```powershell
python 07_implementation/implementation_notes/bl001_bl002_ingestion/export_spotify_max_dataset.py
```

Recommended when PowerShell blocks script sourcing:

```powershell
python 07_implementation/implementation_notes/bl001_bl002_ingestion/export_spotify_max_dataset.py --env-ps1 07_implementation/implementation_notes/bl001_bl002_ingestion/configs/templates/spotify_env_template.ps1
```

Recommended rate-limit-safe run (smaller batches + explicit throttling):

```powershell
python 07_implementation/implementation_notes/bl001_bl002_ingestion/export_spotify_max_dataset.py --env-ps1 07_implementation/implementation_notes/bl001_bl002_ingestion/configs/templates/spotify_env_template.ps1 --batch-size-top-tracks 25 --batch-size-saved-tracks 25 --batch-size-playlists 25 --batch-size-playlist-items 25 --batch-pause-ms 500 --min-request-interval-ms 600 --max-requests-per-minute 70 --max-retries 10
```
...
- spotify_resilience_cache.sqlite (when resilience cache is enabled)

...
DEFAULT_ASSEMBLY_CONTROLS: dict[str, object] = {
    ...
    "pressure_diagnostics": {
        "exclusion_ratio_warn_threshold": 0.3,
        "dominant_reason_share_warn_threshold": 0.7,
    },
}
...
def _build_default_run_config() -> dict[str, Any]:
    return {
        ...
        "profile_controls": {
            ...
            "confidence_input_health": _coerce_object_mapping(DEFAULT_PROFILE_CONTROLS["confidence_input_health"]),
        },
        "retrieval_controls": {
            ...
            "lead_genre_partial_match_threshold": safe_float(DEFAULT_RETRIEVAL_CONTROLS["lead_genre_partial_match_threshold"]),
        ...
        "scoring_controls": {
            ...
            "spread_diagnostics": _coerce_object_mapping(DEFAULT_SCORING_CONTROLS["spread_diagnostics"]),
        },
        ...
        "seed_controls": {
            ...
            "seed_aggregation_health": _coerce_object_mapping(DEFAULT_SEED_CONTROLS["seed_aggregation_health"]),
...
def _validate_bl003_seed_controls(seed_controls: Any, defaults: dict[str, Any]) -> dict[str, Any]:
    ...
    seed_aggregation_health_defaults = dict(defaults.get("seed_aggregation_health") or {})
    seed_aggregation_health_raw = seed_controls.get("seed_aggregation_health")
    if seed_aggregation_health_raw is None:
        seed_aggregation_health_raw = {}
    if not isinstance(seed_aggregation_health_raw, dict):
        raise RunConfigError("run_config.seed_controls.seed_aggregation_health must be an object")
    _validate_allowed_keys(
        seed_aggregation_health_raw,
        set(seed_aggregation_health_defaults),
        "run_config.seed_controls.seed_aggregation_health",
    )
    ...
    return {
        ...
        "seed_aggregation_health": {
            "warn_min_ratio": _validate_fraction_zero_to_one(
                seed_aggregation_health_raw.get("warn_min_ratio"),
                "run_config.seed_controls.seed_aggregation_health.warn_min_ratio",
                float(seed_aggregation_health_defaults.get("warn_min_ratio", 0.01)),
            ),
            "collapse_max_rows": _validate_non_negative_int(
                seed_aggregation_health_raw.get("collapse_max_rows"),
                "run_config.seed_controls.seed_aggregation_health.collapse_max_rows",
                int(seed_aggregation_health_defaults.get("collapse_max_rows", 1)),
            ),
        },
...
def _resolved_path_str(resolved_path: Path | None) -> str | None:
    return str(resolved_path) if resolved_path else None


def _resolve_input_scope_controls_from_effective(
    effective: dict[str, Any],
    resolved_path: Path | None,
) -> dict[str, Any]:
    return {
        "config_path": _resolved_path_str(resolved_path),
        "schema_version": effective["schema_version"],
        "input_scope": dict(effective["input_scope"]),
    }


def _resolve_bl003_influence_controls_from_effective(
    effective: dict[str, Any],
    resolved_path: Path | None,
) -> dict[str, Any]:
...
def _resolve_bl003_seed_controls_from_effective(
    effective: dict[str, Any],
    resolved_path: Path | None,
) -> dict[str, Any]:
    ...
    return {
        ...
        "seed_aggregation_health": dict(seed.get("seed_aggregation_health") or {}),
    }


def _resolve_bl003_weighting_policy_from_effective(effective: dict[str, Any]) -> dict[str, Any]:
    seed = effective["seed_controls"]
...
def _resolve_bl004_controls_from_effective(
    effective: dict[str, Any],
    resolved_path: Path | None,
) -> dict[str, Any]:
    ...
    influence = effective["influence_tracks"]
    confidence_input_health = dict(profile_controls.get("confidence_input_health") or {})
    return {
        "config_source": "run_config" if resolved_path else "defaults",
        "run_config_path": _resolved_path_str(resolved_path),
        "run_config_schema_version": effective["schema_version"] if resolved_path else None,
        "config_path": _resolved_path_str(resolved_path),
        "schema_version": effective["schema_version"],
        ...
        "min_non_fallback_rate_warn_threshold": float(
            confidence_input_health.get("min_non_fallback_rate_warn_threshold", 0.5)
        ),
...
def _resolve_bl011_controls_from_effective(
    effective: dict[str, Any],
    resolved_path: Path | None,
) -> dict[str, Any]:
...
def _resolve_bl011_scenario_policy_from_effective(effective: dict[str, Any]) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    controls = effective["controllability_controls"]
...
def _resolve_bl013_orchestration_controls_from_effective(
    effective: dict[str, Any],
    resolved_path: Path | None,
) -> dict[str, Any]:
...
def resolve_effective_run_config(run_config_path: str | Path | None) -> tuple[dict[str, Any], Path | None]:
    ...
    # Remove legacy/dead schema surface from the effective shape even if older
    # configs still include it, so downstream artifacts stay canonical.
    effective.pop("reporting_controls", None)
    ...
    confidence_input_health_defaults = dict(
        DEFAULT_RUN_CONFIG["profile_controls"].get("confidence_input_health") or {}
    )
    confidence_input_health_raw = profile_controls.get("confidence_input_health")
    if confidence_input_health_raw is None:
        confidence_input_health_raw = {}
    if not isinstance(confidence_input_health_raw, dict):
        raise RunConfigError("profile_controls.confidence_input_health must be an object")
    _validate_allowed_keys(
        confidence_input_health_raw,
        set(confidence_input_health_defaults),
        "profile_controls.confidence_input_health",
    )
    profile_controls["confidence_input_health"] = {
        "min_non_fallback_rate_warn_threshold": _validate_fraction_zero_to_one(
            confidence_input_health_raw.get("min_non_fallback_rate_warn_threshold"),
            "profile_controls.confidence_input_health.min_non_fallback_rate_warn_threshold",
            float(confidence_input_health_defaults.get("min_non_fallback_rate_warn_threshold", 0.5)),
        ),
    }
    ...
    retrieval_controls.pop("signal_mode", None)
    ...
    retrieval_controls["lead_genre_partial_match_threshold"] = _validate_fraction_zero_to_one(
        retrieval_controls.get("lead_genre_partial_match_threshold"),
        "retrieval_controls.lead_genre_partial_match_threshold",
        float(retrieval_defaults["lead_genre_partial_match_threshold"]),
    )
    ...
    scoring_controls.pop("signal_mode", None)
    ...
    spread_diagnostics_defaults = dict(scoring_defaults.get("spread_diagnostics") or {})
    spread_diagnostics_raw = scoring_controls.get("spread_diagnostics")
    if spread_diagnostics_raw is None:
        spread_diagnostics_raw = {}
    if not isinstance(spread_diagnostics_raw, dict):
        raise RunConfigError("scoring_controls.spread_diagnostics must be an object")
    _validate_allowed_keys(
        spread_diagnostics_raw,
        set(spread_diagnostics_defaults),
        "scoring_controls.spread_diagnostics",
    )
    scoring_controls["spread_diagnostics"] = {
        "spread_warn_threshold": _validate_fraction_zero_to_one(
            spread_diagnostics_raw.get("spread_warn_threshold"),
            "scoring_controls.spread_diagnostics.spread_warn_threshold",
            float(spread_diagnostics_defaults.get("spread_warn_threshold", 0.15)),
        ),
    }
    ...
    orchestration_controls = effective.setdefault("orchestration_controls", {})
    if not isinstance(orchestration_controls, dict):
        raise RunConfigError("run_config.orchestration_controls must be an object")
    orchestration_controls.pop("required_stable_artifacts", None)

    assembly_controls = effective.setdefault("assembly_controls", {})
    if not isinstance(assembly_controls, dict):
        raise RunConfigError("run_config.assembly_controls must be an object")
    pressure_diagnostics_defaults = dict(
        DEFAULT_RUN_CONFIG["assembly_controls"].get("pressure_diagnostics") or {}
    )
    pressure_diagnostics_raw = assembly_controls.get("pressure_diagnostics")
    if pressure_diagnostics_raw is None:
        pressure_diagnostics_raw = {}
    if not isinstance(pressure_diagnostics_raw, dict):
        raise RunConfigError("assembly_controls.pressure_diagnostics must be an object")
    _validate_allowed_keys(
        pressure_diagnostics_raw,
        set(pressure_diagnostics_defaults),
        "assembly_controls.pressure_diagnostics",
    )
    assembly_controls["pressure_diagnostics"] = {
        "exclusion_ratio_warn_threshold": _validate_fraction_zero_to_one(
            pressure_diagnostics_raw.get("exclusion_ratio_warn_threshold"),
            "assembly_controls.pressure_diagnostics.exclusion_ratio_warn_threshold",
            float(pressure_diagnostics_defaults.get("exclusion_ratio_warn_threshold", 0.3)),
        ),
        "dominant_reason_share_warn_threshold": _validate_fraction_zero_to_one(
            pressure_diagnostics_raw.get("dominant_reason_share_warn_threshold"),
            "assembly_controls.pressure_diagnostics.dominant_reason_share_warn_threshold",
            float(pressure_diagnostics_defaults.get("dominant_reason_share_warn_threshold", 0.7)),
        ),
    }
...
def resolve_bl004_controls(run_config_path: str | Path | None) -> dict[str, Any]:
    ...
    return _resolve_bl004_controls_from_effective(effective, resolved_path)


def resolve_input_scope_controls(run_config_path: str | Path | None) -> dict[str, Any]:
    effective, resolved_path = resolve_effective_run_config(run_config_path)
    return _resolve_input_scope_controls_from_effective(effective, resolved_path)


def resolve_bl003_influence_controls(run_config_path: str | Path | None) -> dict[str, Any]:
    effective, resolved_path = resolve_effective_run_config(run_config_path)
    return _resolve_bl003_influence_controls_from_effective(effective, resolved_path)


def resolve_bl003_seed_controls(run_config_path: str | Path | None) -> dict[str, Any]:
    effective, resolved_path = resolve_effective_run_config(run_config_path)
    return _resolve_bl003_seed_controls_from_effective(effective, resolved_path)
...
def resolve_bl011_controls(run_config_path: str | Path | None) -> dict[str, Any]:
    effective, resolved_path = resolve_effective_run_config(run_config_path)
    return _resolve_bl011_controls_from_effective(effective, resolved_path)
...
def resolve_bl011_scenario_policy(run_config_path: str | Path | None) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    ...
    return _resolve_bl011_scenario_policy_from_effective(effective)


def resolve_bl003_weighting_policy(run_config_path: str | Path | None) -> dict[str, Any]:
    effective, _ = resolve_effective_run_config(run_config_path)
    return _resolve_bl003_weighting_policy_from_effective(effective)
...
def resolve_bl005_controls(run_config_path: str | Path | None) -> dict[str, Any]:
    ...
    return {
        ...
        "lead_genre_partial_match_threshold": _validate_fraction_zero_to_one(
            retrieval.get("lead_genre_partial_match_threshold"),
            "retrieval_controls.lead_genre_partial_match_threshold",
...
def resolve_bl006_controls(run_config_path: str | Path | None) -> dict[str, Any]:
    ...
    spread_diagnostics = dict(scoring.get("spread_diagnostics") or defaults.get("spread_diagnostics") or {})
    return {
        "config_path": str(resolved_path) if resolved_path else None,
        "schema_version": effective["schema_version"],
        "signal_mode": dict(effective.get("signal_mode") or {}),
        ...
        "spread_warn_threshold": _validate_fraction_zero_to_one(
            spread_diagnostics.get("spread_warn_threshold"),
            "scoring_controls.spread_diagnostics.spread_warn_threshold",
            float((defaults.get("spread_diagnostics") or {}).get("spread_warn_threshold", 0.15)),
        ),
    }
...
def resolve_bl007_controls(run_config_path: str | Path | None) -> dict[str, Any]:
    ...
    pressure_diagnostics_raw = assembly.get("pressure_diagnostics")
    pressure_defaults = defaults.get("pressure_diagnostics") or {}
    pressure_diagnostics = {
        "exclusion_ratio_warn_threshold": _validate_fraction_zero_to_one(
            (pressure_diagnostics_raw or {}).get("exclusion_ratio_warn_threshold")
            if isinstance(pressure_diagnostics_raw, dict)
            else None,
            "assembly_controls.pressure_diagnostics.exclusion_ratio_warn_threshold",
            float(pressure_defaults.get("exclusion_ratio_warn_threshold", 0.3)),
        ),
        "dominant_reason_share_warn_threshold": _validate_fraction_zero_to_one(
            (pressure_diagnostics_raw or {}).get("dominant_reason_share_warn_threshold")
            if isinstance(pressure_diagnostics_raw, dict)
            else None,
            "assembly_controls.pressure_diagnostics.dominant_reason_share_warn_threshold",
            float(pressure_defaults.get("dominant_reason_share_warn_threshold", 0.7)),
        ),
    }
...
def resolve_bl013_orchestration_controls(run_config_path: str | Path | None) -> dict[str, Any]:
    effective, resolved_path = resolve_effective_run_config(run_config_path)
    return _resolve_bl013_orchestration_controls_from_effective(effective, resolved_path)


def resolve_stage_control_bundle(run_config_path: str | Path | None) -> dict[str, dict[str, Any]]:
    effective, resolved_path = resolve_effective_run_config(run_config_path)
    return {
        "BL-003": {
            "input_scope_controls": _resolve_input_scope_controls_from_effective(effective, resolved_path),
            "seed_controls": _resolve_bl003_seed_controls_from_effective(effective, resolved_path),
            "weighting_policy": _resolve_bl003_weighting_policy_from_effective(effective),
            "influence_controls": _resolve_bl003_influence_controls_from_effective(effective, resolved_path),
        },
        "BL-004": _resolve_bl004_controls_from_effective(effective, resolved_path),
        "BL-005": resolve_bl005_controls(run_config_path),
        "BL-006": resolve_bl006_controls(run_config_path),
        "BL-007": resolve_bl007_controls(run_config_path),
        "BL-008": resolve_bl008_controls(run_config_path),
        "BL-009": resolve_bl009_controls(run_config_path),
        "BL-011": {
            **_resolve_bl011_controls_from_effective(effective, resolved_path),
            "scenario_policy": _resolve_bl011_scenario_policy_from_effective(effective)[0],
            "scenario_definitions": _resolve_bl011_scenario_policy_from_effective(effective)[1],
        },
        "BL-013": _resolve_bl013_orchestration_controls_from_effective(effective, resolved_path),
    }

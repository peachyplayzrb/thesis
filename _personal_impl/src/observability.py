...
from run_config.run_config_utils import (
    DEFAULT_CONTROL_MODE as RUN_CONFIG_DEFAULT_CONTROL_MODE,
    DEFAULT_INPUT_SCOPE as RUN_CONFIG_DEFAULT_INPUT_SCOPE,
    DEFAULT_OBSERVABILITY_CONTROLS as RUN_CONFIG_DEFAULT_OBSERVABILITY_CONTROLS,
)
...
DEFAULT_CONTROL_MODE: dict[str, object] = deepcopy(RUN_CONFIG_DEFAULT_CONTROL_MODE)
DEFAULT_INPUT_SCOPE: dict[str, object] = deepcopy(RUN_CONFIG_DEFAULT_INPUT_SCOPE)
DEFAULT_OBSERVABILITY_CONTROLS: dict[str, object] = deepcopy(RUN_CONFIG_DEFAULT_OBSERVABILITY_CONTROLS)
...
def bl009_required_paths(repo_root: Path, bl009_script_path: Path | None = None) -> dict[str, Path]:
    script_path = bl009_script_path or (repo_root / "observability.py")
    return {
...
def sha256_of_values(values: list[str]) -> str:
...
def _extract_config_source(payload: dict[str, object]) -> str:
    source = payload.get("config_source")
    if isinstance(source, str) and source.strip():
        return source.strip()
    config = _object_mapping(payload.get("config"))
...
def _extract_run_config_path(payload: dict[str, object]) -> str | None:
    top_level_path = payload.get("run_config_path")
    if isinstance(top_level_path, str) and top_level_path.strip():
        return top_level_path.strip()
    config = _object_mapping(payload.get("config"))
    config_path = config.get("run_config_path")
...
def main() -> None:
    ...
    upstream_stage_control_sources = {
        "BL-004": _extract_config_source(profile),
        "BL-005": _extract_config_source(bl005_diagnostics),
        "BL-006": _extract_config_source(bl006_summary),
        "BL-007": _extract_config_source(bl007_report),
        "BL-008": _extract_config_source(bl008_summary),
    }
    upstream_stage_run_config_paths = {
        "BL-004": _extract_run_config_path(profile),
        "BL-005": _extract_run_config_path(bl005_diagnostics),
        "BL-006": _extract_run_config_path(bl006_summary),
        "BL-007": _extract_run_config_path(bl007_report),
        "BL-008": _extract_run_config_path(bl008_summary),
    }
    ...
    run_log = {
        "run_metadata": {
            ...
            "upstream_stage_control_sources": upstream_stage_control_sources,
            "upstream_stage_run_config_paths": upstream_stage_run_config_paths,
